...
const Header = () => {
  ...
  const mergedCart = cart.reduce((acc, item) => {
    ...
    if (existingProduct) {
      existingProduct.quantity += item.quantity; // Tăng số lượng nếu sản phẩm trùng
    } else {
      acc.push({ ...item }); // Thêm sản phẩm mới vào giỏ hàng
    }
    return acc;
  }, []);
        ...
        <div className="container-right">
          <div className="action-header flex items-center gap-10">
            <div className="cart cursor-pointer relative hidden md:block group">
              ...
              <Badge count={mergedCart.reduce((total, item) => total + item.quantity, 0)} offset={[2, 0]} showZero>
                <ShoppingCartOutlined className='text-3xl text-mainColor1' />
              </Badge>

              <div className="mini-cart min-w-[200px] absolute left-[-20px] p-4 border-[1px] border-borderColor1 rounded-md bg-bodyColor z-[99] shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out hidden group-hover:block">
