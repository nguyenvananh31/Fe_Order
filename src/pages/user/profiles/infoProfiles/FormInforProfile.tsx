...
const FormInforProfile = ({ profile, isEditingAddresses }: any) => {
  ...
  const handleViewDetail = (order: any) => {
    // Điều hướng đến trang profile/bill và truyền ID của đơn hàng
              ...
              <Collapse accordion>
                {profile?.addresses?.map((address: any, index: number) => (
                    ...
                    >
                      ...
                      <p>
                        <strong>Mặc Định:</strong> {address.is_default ? "Có" : "Không"}
                      </p>
                    </Panel>
                ))}
              </Collapse>
                        ...
                        {fields.map(({ key, name, fieldKey, ...restField }) => (
                            <Collapse key={key} accordion>
                              <Panel header={`Địa chỉ ${name + 1}`} key={key}>
                                ...
                                >
                                  <Checkbox
                                      onChange={(e) => handleDefaultChange(name, e.target.checked)}
                                  ></Checkbox>
                                </Form.Item>
